...
model_checkpoints_dir = Path(__file__).resolve().parent.parent.joinpath("checkpoints")
...
def initialize_clip(config, num_patches=240):
    ...
    if config["clip_name"] == "ViT-B-16":
        clip_model, preprocess = clip.load(model_checkpoints_dir.joinpath("ViT-B-16.tar"), jit=False)
        num_patches = int(config['image_res'] * config['image_res'] / (16 * 16))
        pos_embed = nn.Parameter(torch.zeros(num_patches + 1, 768).float())
    elif config["clip_name"] == "ViT-L-14":
        clip_model, preprocess = clip.load(model_checkpoints_dir.joinpath("ViT-L-14.tar"), jit=False)
